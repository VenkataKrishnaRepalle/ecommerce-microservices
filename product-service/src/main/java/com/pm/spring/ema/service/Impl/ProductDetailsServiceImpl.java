...
public class ProductDetailsServiceImpl implements ProductDetailsService {
  ...
  public List<ProductDetailsFindResponseDto> create(
    ...
    for (int count = 0; count < detailNames.length; count++) {
      if (!detailNames[count].isEmpty() && !detailValues[count].isEmpty()) {
        ProductDetailsCreateRequestDto isProductDetailsExists =
            productDetailsMapper.ProductDetailsToProductDetailsCreateRequestDto(
        ...
        ProductDetailsCreateRequestDto productDetailsRequestDto =
            new ProductDetailsCreateRequestDto();
    ...
    for (ProductDetailsCreateRequestDto productDetail : productDetails) {
      ProductDetailsFindResponseDto finalProductDetails =
          productDetailsMapper.productDetailsToProductDetailsFindResponseDto(
              productDetailsDao.save(
                  productDetailsMapper.ProductDetailsCreateRequestDtoToProductDetails(
                      productDetail)));
  ...
  public List<ProductDetailsFindResponseDto> update(
      ...
      for (int index = 0; index < detailName.length; index++) {
        if (!detailName[index].isEmpty() && !detailValue[index].isEmpty()) {
          ...
          if (productDetails == null) {
            ...
            ProductDetails pd =
                productDetailsDao.save(
                    productDetailsMapper.ProductDetailsUpdateRequestDtoToProductDetails(
                        productDetails1));
    ...
    for (ProductDetails listProductDetail : listProductDetails) {
      ProductDetailsFindResponseDto finalProductDetails =
          productDetailsMapper.productDetailsToProductDetailsFindResponseDto(listProductDetail);
      productDetailsFindResponseDtoList.add(finalProductDetails);
    }
    return productDetailsFindResponseDtoList;
  }
  ...
  public List<ProductDetailsFindResponseDto> getByProductId(UUID id) {
    ...
    return productDetailsDao.getByProductId(id).stream()
        .map(productDetailsMapper::productDetailsToProductDetailsFindResponseDto)
        .toList();
  }
