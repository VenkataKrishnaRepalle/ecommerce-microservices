...
public class ProductDetailsServiceImpl implements ProductDetailsService {
  ...
  public List<ProductDetailsDto> update(UUID productId, String[] detailName, String[] detailValue) {
      ...
      for (int index = 0; index < detailName.length; index++) {
        if (!detailName[index].isEmpty() && !detailValue[index].isEmpty()) {
          ...
          if (productDetails == null) {
            ...
            ProductDetails pd =
                productDetailsDao.save(productDetailsMapper.toProductDetails(productDetailsDto));
