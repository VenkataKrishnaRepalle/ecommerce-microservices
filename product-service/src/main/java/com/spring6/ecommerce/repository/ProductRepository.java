...
public interface ProductRepository extends JpaRepository<Product, UUID> {
    ...
    @Query("SELECT p FROM Product p WHERE p.categoryId = :categoryId AND p.brandId = :brandId ORDER BY p.categoryId, p.brandId")
    List<Product> getByCategoryIdAndBrandId(@Param("categoryId") UUID categoryId, @Param("brandId") UUID brandId);
}
