...
public class CustomerServiceImpl implements CustomerService {
  ...
  public ApiResponse<Void> login(LoginDto loginDto) {
    ...
    if (customer.getEmail().equals(loginDto.getEmail())
        && passwordEncoder.matches(loginDto.getPassword(), customer.getPassword())) {
    ...
    if (customer != null
      ...
      if (passwordEncoder.matches(forgotPasswordDto.getPassword(), customer.getPassword())) {
        return ApiResponse.error(
            "Entered password is same as old password, Please Enter a new Password");
      }
  ...
  public ApiResponse<Void> changePassword(ChangePasswordDto changePasswordDto) {
    ...
    System.out.println(customer);
    if (customer != null
        && passwordEncoder.matches(changePasswordDto.getOldPassword(), customer.getPassword())) {
      if (changePasswordDto.getOldPassword().equals(changePasswordDto.getNewPassword())) {
        return ApiResponse.error(
            "Old Password and New password Matched, Please Enter a different new Password");
      } else if (changePasswordDto
          .getOldPassword()
          .equals(changePasswordDto.getConfirmNewPassword())) {
        return ApiResponse.error(
            "Old Password and Confirm New password Matched, Please Enter a different Confirm New Password");
      } else if (changePasswordDto
          .getNewPassword()
          .equals(changePasswordDto.getConfirmNewPassword())) {
        ...
        && !passwordEncoder.matches(changePasswordDto.getOldPassword(), customer.getPassword())) {
      return ApiResponse.error(
          "Entered Old Password is incorrect, Please provide correct Old Password");
    } else {
  ...
  public String getCountryCode(String countryName) {
    CountryDto country =
        countryMapper.countryToCountryFindResponseDto(
            countryRepository.getCountryByName(countryName.toLowerCase()));
