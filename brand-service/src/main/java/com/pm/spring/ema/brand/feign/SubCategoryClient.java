...
@FeignClient(
    name = "spring6-subcategory",
    url = "localhost:1006/api/sub-category",
    configuration = CustomFeignConfiguration.class)
public interface SubCategoryClient {

  @GetMapping("/{subCategoryId}")
  SubCategoryDto getById(
      @RequestHeader("Authorization") String token, @PathVariable UUID subCategoryId);
}
